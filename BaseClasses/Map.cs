...
namespace LightRise.BaseClasses
{
    public class Map
    {
        ...
        public const uint LSHELF = 2;
        public const uint RSHELF = 3;
            ...
            for (uint i = 0; i < Width; i++)
            {
                for (uint j = 0; j < Height; j++)
                {
                    Grid[i][j] = SimpleUtils.Choose(new Tuple<uint, float>[] {
                        ...
                        new Tuple<uint, float>(LSHELF, 2),
                        new Tuple<uint, float>(RSHELF, 2)});
                }
            }
        }
        ...
        public virtual void Draw(SpriteBatch spriteBatch, Camera cam)
        {
                ...
                i < iTo; i++)
            {
                    ...
                    j < jTo; j++)
                {
                    ...
                    if (this[i, j] == LSHELF)
                    {
                        spriteBatch.Draw(SimpleUtils.WhiteRect, new Rectangle(cam.WorldToWindow(new Vector2(i, j)), cam.Scale.CeilingToPoint()), Color.Yellow);
                    }
                    if (this[i, j] == RSHELF)
                    {
                        spriteBatch.Draw(SimpleUtils.WhiteRect, new Rectangle(cam.WorldToWindow(new Vector2(i, j)), cam.Scale.CeilingToPoint()), Color.Blue);
                    }
                }
            }
        }
